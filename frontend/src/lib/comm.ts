...
export type ServerMessage = HubHello | LobbyHello | NewPlayer | StartGame | ProgressUpdate | PlayerFinished;
...
function parseWord(view: DataView, offset: number): [string, number] {
    ...
    const wordBytes = new Uint8Array(view.buffer, view.byteOffset + offset, wordLen);
...
function parsePlayer(view: DataView, offset: number): [Player, number] {
    ...
    return [{ id: playerId, name, statusEffects: [], finished: false, progress: 0 }, offset];
}
...
async function connect_raw(url: string): Promise<Socket> {
    ...
    return {
        ...
        sendRegister: (name: string) => { socket.send(serializeClientMessage({ opcode: ClientOp.Register, name })) },
