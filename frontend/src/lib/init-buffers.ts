...
function initBuffers(gl:WebGLRenderingContext, id_of_file:string, id_of_material_file:string = ""): InitBufferReturn|undefined {
  var objStr:string|undefined = document.getElementById(id_of_file)?.innerHTML;
  if(objStr === undefined) 
    {alert("Could not find string to init buffers! string was:"+ id_of_file);return;}
  
  var mesh:OBJ.Mesh = new OBJ.Mesh(objStr);
  OBJ.initMeshBuffers(gl, mesh);
  
  const positionBuffer:OBJ.ExtendedGLBuffer = (mesh as OBJ.MeshWithBuffers).vertexBuffer;
  const textureCoordBuffer:OBJ.ExtendedGLBuffer = (mesh as OBJ.MeshWithBuffers).textureBuffer;
  const indexBuffer:OBJ.ExtendedGLBuffer = (mesh as OBJ.MeshWithBuffers).indexBuffer;
  const normalBuffer:OBJ.ExtendedGLBuffer = (mesh as OBJ.MeshWithBuffers).normalBuffer;

  // Material Stuff
  const materialIndexBuffer:number[] = mesh.vertexMaterialIndices;
  const materialNameToIndexBuffer = mesh.materialIndices
  const materials = initMaterials(id_of_material_file);
...
function initMaterials(id_of_material_file:string) {
  // If don't have a material file, just return
  if(id_of_material_file == "")
    return {};
  
  const mtlString = document.getElementById(id_of_material_file)?.innerHTML;
  if(mtlString === undefined) 
    {alert("Could not find string to init material file! string was:"+ id_of_material_file);return {};}
