...
import type { LobbyHello, NewPlayer, Player, Socket, StatusEffectId, StartGame } from './lib/comm.ts';
...
function connect(
    ...
    return async (name: string) => {
        ...
        socket.event.onHubHello((_) => { console.log("received hub hello"); });
        ...
        console.log(socket);
        socket.socket.addEventListener('open', (_) => socket.sendRegister(name));
...
export function PageProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
      ...
      connect(setSocket, setPlayers, setProgress, setStatusEffects, setWords, setPage)(name);
      return () => {}
  }, [name]);
  return (
